@echo off
rem author: keith cascio, since: 20060524
...
set JAVA_OPS=-Xms64m -Xmx512m -cp %CPATH%

if exist "jvm_profiler.dll" (
  set JAVA_OPS=-Xrunjvm_profiler %JAVA_OPS%
) else (
  echo.
  echo ####################################################################
  echo # warning: optional profiler library "jvm_profiler.dll" not found. #
...
@echo on
"%JRE14BINPATH%\java.exe" %JAVA_OPS% %MAIN_CLASS_CMD% %*
@echo off
goto end
