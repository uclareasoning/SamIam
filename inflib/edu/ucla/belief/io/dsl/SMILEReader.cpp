...
int LoadNode( char* cname, char* cid, DSL_idArray* states, jint* jint_args,
...
int LoadSubmodel( jint jhandle, char* cname, jint jxcoord, jint jycoord, DSL_intArray* children )
{
    //setup String arguments(s)
    jstring jname = theEnvironment->NewStringUTF( cname );
...
int LoadSubmodel( const int handle )
{
	...
	if( submodel )
	{
		char* tempSubmodelName = submodel->header.GetName();
...
#endif
	theEnvironment->ThrowNew( class_Exception, "DSL_SYNTAX_ERROR" );
	return EXCEPTION_OCCURRED;
    }
