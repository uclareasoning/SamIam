...
foo="${JDK15BASEPATH:?}${SMILEPATH:?}";
export JDK15BASEPATH SMILEPATH;

cd `dirname $0`

# Define default values for environment variables used in the makefiles.
programOutput="libcallsmile.so"
defaultOS=""
defaultOSArch=""
defaultWS="gtk"
makefile=""
...
# "fail on unresolved symbol references except for ___tls_get_addr".
# ___tls_get_addr comes from /lib/ld-linux.so.2 at dynamic link time.
...
if [ "$makefile" != "" ]; then
  if [ "$extraArgs" != "" ]; then
    ...
    make -f $makefile all    2> >(sed -re '/libstdc++.*eh_globals/N;/warning.*___tls_get_addr/c \
