 ...
 * @author Nobuo Tamemasa 10/20/98
...
public class GroupableTableHeaderUI extends BasicTableHeaderUI {
    ...
    public static int getHeaderHeight( JTableHeader jtableheader ){
    	int height = 0;
    	JTable jtable = jtableheader.getTable();
    	GroupableTableColumnModel columnModel = (GroupableTableColumnModel)jtableheader.getColumnModel();
    	TableColumn aColumn = null;
    	int columnCount = columnModel.getColumnCount();
    	for( int column = 0; column < columnCount; column++ ){
			aColumn = columnModel.getColumn(column);
			height = Math.max( height, getHeaderHeight( jtable, jtableheader, aColumn, columnModel ) );
		}
		return height;
    }
    ...
    /** @author Keith Cascio
    	@since 110204 election day */
    public static int getHeaderHeight( JTable jtable, JTableHeader jtableheader, TableColumn aColumn, GroupableTableColumnModel columnModel ){
    	TableCellRenderer renderer = aColumn.getHeaderRenderer();
    	if( renderer == null ) renderer = jtableheader.getDefaultRenderer();
		Component comp = renderer.getTableCellRendererComponent( jtable, aColumn.getHeaderValue(), false, false,-1, aColumn.getModelIndex() );
		int cHeight = comp.getPreferredSize().height;
		Iterator iter = columnModel.getColumnGroups(aColumn);
		if( iter != null ){
			ColumnGroup cGroup;
			while( iter.hasNext() ){
				cGroup = (ColumnGroup) iter.next();
				cHeight += cGroup.getSize( jtable ).height;
			}
		}
		return cHeight;
    }
